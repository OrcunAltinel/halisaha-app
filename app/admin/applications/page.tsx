...
function StatusBadge({ status, locale }: { status: Application['status']; locale: Locale }) {
  const styles = {
  ...
  const label = {
    pending: t(locale, 'Pending', 'Beklemede'),
    approved: t(locale, 'Approved', 'Onaylandı'),
    rejected: t(locale, 'Rejected', 'Reddedildi'),
  }[status]
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white md:text-4xl">
            {t(locale, 'Review Applications', 'Başvuruları İncele')}
          </h1>
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            {t(locale, 'Review turf owner applications and approve or reject them.', 'Saha sahibi başvurularını incele, onayla veya reddet.')}
          </p>
        </div>
        ...
        {error && (
          <div className="mb-6 rounded-xl bg-red-50 dark:bg-red-900/20 p-4 text-sm text-red-700 dark:text-red-400 ring-1 ring-red-200 dark:ring-red-800">
            {t(locale, 'Failed to load applications. Please refresh the page.', 'Başvurular yüklenemedi. Lütfen sayfayı yenile.')}
          </div>
        )}
        ...
        {!error && applications.length === 0 && (
          <div className="rounded-2xl bg-white dark:bg-gray-900 p-8 text-center shadow-sm ring-1 ring-gray-100 dark:ring-gray-800">
            <p className="text-base font-medium text-gray-900 dark:text-white">
              {locale === 'tr' ? `${tabLabels[activeStatus]} başvuru yok` : `No ${activeStatus} applications`}
            </p>
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              {activeStatus === 'pending'
                ? t(locale, 'Nothing to review right now. New applications will appear here.', 'Su an incelenecek bir sey yok. Yeni başvurular burada görünecek.')
                : locale === 'tr'
                ? `Henüz ${tabLabels[activeStatus].toLowerCase()} başvuru yok.`
              ...
              >
                <div className="p-5 sm:p-6">
                  ...
                  <div className="mt-3 grid gap-2 text-sm sm:grid-cols-2">
                    <div>
                      <span className="text-gray-500 dark:text-gray-400">{t(locale, 'Applicant:', 'Başvuran:')} </span>
                      <span className="font-medium text-gray-900 dark:text-white">
                        {app.user_email || app.user_id}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-500 dark:text-gray-400">{t(locale, 'Submitted:', 'Gönderilme:')} </span>
                      <span className="font-medium text-gray-900 dark:text-white">
                        {formatDate(app.created_at, locale)}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-500 dark:text-gray-400">{t(locale, 'Price:', 'Ücret:')} </span>
                      <span className="font-medium text-gray-900 dark:text-white">
                        ₺{app.price_per_hour} {t(locale, '/ hour', '/ saat')}
                      </span>
                    </div>
                    {app.phone && (
                      <div>
                  ...
                  {app.image_urls && app.image_urls.length > 0 && (
                    <div className="mt-4">
                      <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                        {t(locale, 'Photos', 'Fotoğraflar')} ({app.image_urls.length})
                      </p>
                      <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
                        {app.image_urls.map((url, idx) => (
                          ...
                          >
                            ...
                            <img
                              src={url}
                              alt={locale === 'tr' ? `Fotoğraf ${idx + 1}` : `Photo ${idx + 1}`}
                  ...
                  {app.status !== 'pending' && (
                    <div className="mt-4 border-t border-gray-100 dark:border-gray-800 pt-4 text-sm">
                      <p className="text-gray-500 dark:text-gray-400">
                        {t(locale, 'Reviewed', 'Incelendi')}{' '}
                        {app.reviewed_at ? formatDate(app.reviewed_at, locale) : t(locale, 'recently', 'az once')}
                      </p>
                      {app.review_notes && (
                        <div
                        ...
                        >
                          <p className="font-semibold">{t(locale, 'Notes', 'Notlar')}</p>
                      ...
                      {app.status === 'approved' && app.approved_turf_id && (
                        ...
                        >
                          {t(locale, 'View live pitch →', 'Canlı sahayı gör →')}
                        </Link>
                      )}
                    </div>
                  )}
