    ...
    if (error) {
      const msg =
        error.message === 'slot_not_available'
          ...
          ? 'Another team already has a pending challenge on that slot.'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm">
        {(['Pick a pitch', 'Pick a slot', 'Confirm'].map((label, i) => {
      ...
      {step === 1 && (
        <div>
          ...
          {turfsLoading ? (
            <p className="text-sm text-gray-500 dark:text-gray-400">Loading pitches…</p>
          ) : turfs.length === 0 ? (
            <p className="text-sm text-gray-500 dark:text-gray-400">No pitches available.</p>
          ) : (
      ...
      {step === 3 && selectedTurf && selectedSlot && (
        <div className="space-y-5">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Confirm challenge</h2>
          </div>
          ...
          <div className="rounded-2xl bg-white dark:bg-gray-900 p-5 shadow-sm ring-1 ring-gray-100 dark:ring-gray-800 space-y-2 text-sm">
            <p>
              <span className="font-semibold text-gray-700 dark:text-gray-300">Challenging:</span>{' '}
              <span className="text-gray-900 dark:text-white">{challengedTeamName}</span>
            </p>
            <p>
              <span className="font-semibold text-gray-700 dark:text-gray-300">Pitch:</span>{' '}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={300}
              placeholder="Add a note to the opposing team…"
